  ...
  userOpts = {
    ...
    options = {
      ...
      files = mkOption {
        ...
        description = ''
          Set of files that have to be linked to the user's home directory.
        '';
      };
    };
...
in {
  ...
  disabledModules = ["config/users-groups.nix"];
  ...
  in {
    ...
    systemd = {
      user.tmpfiles.users = mapAttrs' (_: {
        name,
        files,
        home,
        ...
        ...
        ) (filter (f: f.enable) (attrValues files));
      }) (filterAttrs (_: u: u.files != {}) cfg.users);

      services.linger-users = lib.mkIf ((length lingeringUsers) > 0) {
